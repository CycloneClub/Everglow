...
4. 反复调用 `dotnet build`（尤其是被脚本或自动化代理批量调用）后，MSBuild 的常驻 worker 节点进程不会随构建结束而退出，会不断累积并占用内存

	- 现象：任务管理器中存在多个 `dotnet.exe`（有时还伴随 `MSBuild.exe` / `VBCSCompiler.exe`），内存占用持续增长；即使当前没有任何构建任务在运行，这些进程依然存在。节点进程的命令行形如 `dotnet.exe "…\MSBuild.dll" /nodemode:1 /nodeReuse:true`，数量默认等于 CPU 核数
	- 原因：MSBuild 默认启用节点复用（`nodeReuse:true`），构建结束后节点进程会驻留等待下次复用，短时间内的连续/批量构建会让它们持续堆积
	- 处理：批量构建结束后执行以下命令即可优雅关闭 MSBuild 与编译器服务器（返回码 0 表示成功）：
		...
		```
	- 预防：如需临时禁用节点复用，可在构建时附加 `/nodeReuse:false`，或设置环境变量 `MSBUILDDISABLENODEREUSE=1`。不要为此修改项目构建配置（`Directory.Build.props` / `*.targets`）
