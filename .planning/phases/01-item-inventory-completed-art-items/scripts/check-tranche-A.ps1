<#
.SYNOPSIS
	Tranche-A coverage and no-placeholder gate (Phase 1 completed-art items).

.DESCRIPTION
	Selects every entry in 01-INVENTORY.json with phase == 1, tranche == "A" and
...
$selected = @($entries | Where-Object {
	([int]$_.phase -eq 1) -and ([string]$_.tranche -eq 'A') -and ([bool]$_.artwork_complete)
})

$failures = New-Object System.Collections.Generic.List[string]

if ($selected.Count -eq 0) {
	Write-Output 'FAIL: tranche-A phase-1 completed-art selection is empty'
	exit 1
}
...
Write-Output "OK(0): tranche-A covered entries = $covered (of $($selected.Count) selected)"
exit 0
