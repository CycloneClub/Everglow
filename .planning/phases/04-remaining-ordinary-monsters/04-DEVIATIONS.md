...
| D-55 | applied, with the section 14.7 review-fix correction | every state transition is written under `Main.netMode != NetmodeID.MultiplayerClient` with `NPC.netUpdate`, every parameter a client must agree on lives in `NPC.ai[]` behind named wrappers (`NPC.localAI[]` never reaches a client - the five flags that were mistakenly placed there were moved to `NPC.ai[]` by section 14.7), and every graphics path sits inside `!Main.dedServ`; section 5 |
...
### 14.7 Review-fix register (/gsd-code-review 04 --fix, iteration 1)

The post-close-out code review wrote `.planning/phases/04-remaining-ordinary-monsters/04-REVIEW.md` (2 Critical, 3 Warning, 5 Info) and the default Critical-and-Warning fix pass ran on 2026-09-16. This section is the register: what each in-scope finding changed, the commits, the decisions the fixes added, and what was deliberately left open. Full report: `04-REVIEW-FIX.md`; the review's own resolution table is in `04-REVIEW.md` under `## Fix Resolution`.

| Finding | Commit | Change | New decision recorded |
| --- | --- | --- | --- |
| CR-01 | `b7d134fe0` | `ToxicToad.OnHitPlayer` and `ToxicToad_PoisonBubble.OnHitPlayer` roll the 75 % branch with `Main.rand.NextBool(3, 4)` (the "X out of Y" overload) instead of `Main.rand.NextBool(3)` (1 in 3 = 33 %). Both doc comments corrected. | none - a straight defect correction; the split is now the design's 75 % 中毒 (600 ticks) / 25 % `BuffID.Venom` (420 ticks) |
| CR-02 | `dd2572f48` | `AnimatedWitherbarkHound` / `AnimatedWitherbarkSoldierRanged` / `AnimatedWitherbarkSoldierSpell` `ProvokedFlag` `localAI[1]` -> `ai[1]`; `SailfinSnakehead` `AggroTimer` `localAI[0]` -> `ai[1]` and `ChargeCooldown` `localAI[1]` -> `ai[2]`; `ArmoredShrimp` `Heading` `localAI[1]` -> `ai[2]`. `SailfinSnakehead.UpdateTimers` runs both counters on every side; the `netUpdate` broadcast stays authoritative. | (a) The review's `NPC.ai[]` route was chosen over `SendExtraAI`/`ReceiveExtraAI`: tML syncs all four `ai[]` slots on `netUpdate`, which the classes already set, and no new serialisation contract is needed. (b) `localAI` is reserved for authoritative-only timers whose value no client branch reads (the three `StateTimer`/`CastTimer` counters stay there). (c) The stale "pushes the new values to the clients through the synced `NPC.localAI[]`" doc comments are corrected, since `localAI[]` is not synchronised. |
| WR-01 | `0c79bd497` | `LargeMossyThornTurtle.ApplyStateMotion` re-derives `NPC.noTileCollide` / `NPC.noGravity` from the synced `State` on every side; `EnterState` keeps its authoritative write. | `noTileCollide` / `noGravity` are not part of the NPC net message, so any state that changes them must re-assert them from synced state on every side (the section 5 doctrine widened by one clause). |
| WR-02 | `86f2ff1f9` | `AnimatedWitherbarkSoldierSpell.TeleportNear` bottom-aligns the 46-px body on the validated floor (`(tileY + 1) * 16f - NPC.height / 2f`) instead of using the candidate tile's centre, which buried the feet 15 px into the solid tile `IsStandableSpot` required. | A teleport's placement must be bottom-aligned on the floor its clearance check validated; `IsStandableSpot` itself is unchanged. |
| WR-03 | `043ce97c5` | `RedNeedleCaterpillar.UpdateVolley` anchors the volley on the synced `NPC.Center` instead of `NPC.Center + Segments[0].SelfPosition`; the `Segments.Count == 0` guard that existed only for that index is removed. | The `Caterpillar` template's segment simulation is per-side (it consumes `Main.rand` on every side and is never synchronised), so the volley cannot use its head offset. The design's 在头部发射 is met by intent only: the synced centre is the closest deterministic anchor, and the worm's per-side rendered head sits about half a body length from it. This is a deliberate, recorded deviation (D-54/D-55), not an oversight. |

**Decisions deliberately not taken.** Two Critical-tier fixes change runtime branch/sync behaviour rather than syntax, so they are marked `fixed: requires human verification` in `04-REVIEW-FIX.md`: only a multiplayer observation can show that a client's AI branch now matches the server (`CR-02`) and that the state-3 dive and the volley origin agree on both sides (`WR-01`, `WR-03`). Those observations remain part of the not-executed D-21 bundle of `04-UAT.md`.

**Info findings left open (out of the requested scope).** `IN-01` (unused `using` directives), `IN-02` (`WaterStrider.DashAngle` dead state), `IN-03` (redundant `NPC.netUpdate` on a projectile-only spawn), `IN-04` (the three D-45 shells have no water test) and `IN-05` (`GlowSalamander`'s global-tick suffocation cadence). They are unaffected by the five fixes and are listed in `04-REVIEW-FIX.md` for a later pass.
