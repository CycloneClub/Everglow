...
- [x] **ITEM-04**: Quest-exclusive, NPC-trade, settlement-restoration, purification, mission rewards, and biology-design drops/materials used by those progression paths are implemented with the documented unlock conditions and progression dependencies.
...
**Phase 4 close-out (2026-09-16).** Phase 4 implemented the **remaining ordinary-creature tranche** of BIO-01, BIO-02 and BIO-03 - all 21 non-deferred `phase: 4` rows of the shared matrix (`03-BIOLOGY.json`, tranche rule D-44) - across **26 `ModNPC` classes** and eleven enemy projectiles. That completes the code side of every ordinary creature in the three regions: 13 of 13 Death Jade Lake rows (BIO-01), 3 of 3 Spiny Moss Court rows (BIO-02) and 5 of 5 Valley of Lush and Moist rows (BIO-03), on top of Phase 3's five repository-art rows. Four Phase 1 drop types (`RadialCarapace`, `MeatLantern`, `Photophore`, `ActivatedDogStaff`) plus the pre-existing cross-namespace `CaterpillarJuice` are wired; every absent drop, absent capture item, unimplemented system (Spiny Moss Court morale/command, Valley egg laying, Valley disguised hazards), region predicate (森雨幽谷 / 刺苔庭园) and effect gap is recorded as a matrix blocker and consolidated in `04-DEVIATIONS.md` section 14.1/14.5; and **no item scope was promoted into the phase** (D-57/D-58).

**Why the statuses still do not change.** BIO-01/BIO-02/BIO-03 are complete on the code side, but this milestone marks a requirement complete only when its verification half lands as well: the D-21 client and dedicated-server bundle in `04-UAT.md` is **recorded and not executed** (24 checks), and QUAL-04's per-item Feishu synchronization belongs to Phase 8. BIO-06 and QUAL-03 therefore keep their structural halves only (subworld-isolated spawn gates, dedicated-server guards), and QUAL-01/QUAL-02/QUAL-03/QUAL-04 stay in Phase 8. ITEM-07 (localization) stays deferred by user directive (D-20): no exporter was run, no key was fabricated and no HJSON was edited. QUAL-05 remains complete - the phase's blockers, deviations, chain evidence and UAT bundle are recorded in `03-BIOLOGY.json`/`.md`, `04-DEVIATIONS.md`, `04-UAT.md` and `04-VALIDATION.md` without touching the Feishu design source (D-25). The whole offline chain is green in one run (Release build 0 warnings / 0 errors; the Phase 4 and Phase 3 gates and the Phase 1/2 gates all exit 0; 3 Yggdrasil tests pass).
...
*Last updated: 2026-09-16 - Phase 4 close-out recorded: all 21 remaining ordinary-creature rows are code-complete across 26 classes, the consolidated ledger, the UAT bundle and the validation contract are written, and the full offline chain is green; no requirement status changed*
