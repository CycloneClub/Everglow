...
/// <summary>
/// 布罗迪蝇蜓（小）（Small Brodie Flydragon）——刺苔庭园（Spiny Moss Court）与森雨幽谷（Valley of Lush and Moist）的飞行敌怪（小）.
...
/// <para>
/// This is a <b>sibling class</b> of the row whose <c>internal_name</c> is <c>BrodieFlydragon</c> (the 标准 class): the
/// design gives 普通（小）its own stat row (生命 20 / 伤害 15 / 防御 2 / 击退抗性 50 / 免疫 中毒 / 0 copper), and
/// <c>SetDefaults</c> runs once per NPC type, so a stat variant must never be faked by mutating <c>NPC.lifeMax</c> at
/// runtime (OQ2, 04-DEVIATIONS.md section 2).
/// </para>
/// <para>
/// 会在森雨幽谷大面积产卵 ... 在森雨幽谷打破卵后会刷新小型的蝇蜓: D-46 BLOCKER - the Valley egg system does not exist.
/// There is no egg NPC, no egg-breaking spawn path and no egg class in this plan, and this class does not rely on one:
/// the small variant is reachable through its own lower conservative natural spawn weight, so the design's creature is
/// present in the layer while the egg route stays a recorded deferral rather than an approximation. 自然刷新只会刷新
/// 标准大小的蝇蜓 is preserved because the standard class carries the higher of the two weights
/// (04-DEVIATIONS.md sections 5, 7, 13).
/// </para>
/// <para>
/// Approved artwork is missing from the repository, so the class requests the shared <c>Commons.ModAsset.White_Mod</c>
/// fallback. The <c>SpinyMossCourt</c> region subfolder is what tML's default (namespace-derived) texture resolution
/// expects, so the D-49 migration is adding <c>SmallBrodieFlydragon.png</c> beside this .cs and deleting the
/// <c>Texture</c> override.
/// </para>
/// </summary>
public class SmallBrodieFlydragon : ModNPC
{
	...
	private const float HoverBobSpeed = 0.5f;

	/// <summary>
	/// 小型的飞行敌怪: the small variant's own lower natural spawn weight. It exists so the design's creature is
	/// present in the layer while the 森雨幽谷 egg route stays an unimplemented D-46 system (04-DEVIATIONS.md
	/// section 10).
	/// </summary>
	private const float SmallFlydragonWeight = 0.5f;
	...
	/// <summary>Named wrapper over <c>NPC.localAI[0]</c>: the frames spent in the current hover cycle.</summary>
	...
	public override void SetStaticDefaults()
	{
		...
		// 免疫 中毒 (the design row's 免疫 cell for the 小 row): the poisoned debuff is the only immunity.
	...
	public override void SetDefaults()
	{
		...
		// No sprite exists to measure, so a conservative small flying box is used (D-54 default).
		NPC.width = 30;
		NPC.height = 22;
		...
		NPC.value = 0; // 钱币（铜） 0, the design's own 小 row value
	...
	/// <summary>
	/// The open-air hover: a lazy patrol with a vertical bob that turns after <see cref="HoverCycleFrames"/> or at a
	/// wall, and hands over to the pursuit as soon as a player is inside <see cref="AggroRange"/>.
	/// </summary>
	...
	/// <summary>
	/// 主动靠近玩家: fly at the player until the melee range is reached (the press), or until the pursuit has gone on
	/// for <see cref="PursueGiveUpFrames"/> without a hit, in which case the creature disengages.
	/// </summary>
	...
	/// <summary>
	/// 进行近战攻击: press into the player for <see cref="LungeFrames"/> so the creature's contact damage lands, then
	/// back out into the pursuit.
	/// </summary>
	...
	/// <summary>
	/// The small variant's own lower conservative land weight. The design's route to this creature is
	/// 在森雨幽谷打破卵, and that 森雨幽谷 egg system is an unimplemented D-46 blocker, so the small variant carries a
	/// natural weight below the standard class's instead of an approximated egg path - which also preserves
	/// 自然刷新只会刷新标准大小的蝇蜓. Only ever inside Yggdrasil and inside the Kelp Curtain layer (BIO-06):
	/// <c>NPCSpawnManager.EditSpawnPool</c> returns early outside the subworld, so this per-creature gate is the real
	/// isolation, and <see cref="KelpCurtainBiome.IsKelpCurtainLayer"/> is the server-safe layer predicate because the
	/// hook runs in single player or on the server only, where the client camera is zero (D-52/D-55). The design's
	/// 刺苔庭园 / 森雨幽谷 split is a Phase 5-6 region blocker and is never approximated with a coordinate range (D-53).
	/// </summary>
